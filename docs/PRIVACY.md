...
Open FullScreenshot is designed to need nothing beyond the browser tab you point it at.
...
menu), Open FullScreenshot reads the pixels of the current tab — either the visible
...
Nothing. Open FullScreenshot makes zero network requests. There is no server, no API
...
|---|---|
| `activeTab` | Grants temporary access to the tab you're currently on, only when you invoke the extension. This is what lets `tabs.captureVisibleTab` and the on-demand content-script injection work, and it expires the moment you navigate away. Open FullScreenshot deliberately does **not** request a host permission (e.g. `<all_urls>`), which would let it read every website you visit at any time. |
| `scripting` | Lets the extension inject its capture/selection scripts into the current tab on demand. There are no scripts running automatically in the background on pages you haven't asked Open FullScreenshot to act on. |
...
Open FullScreenshot has no support server or telemetry channel by design. If you have a
